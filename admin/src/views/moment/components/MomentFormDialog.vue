<template>
        ...
        >
          ...
          <template v-else-if="preview.type === 'video'">
            ...
            <div
              ...
              v-if="!preview.isLocal && preview.url"
            >
              <iframe
                :src="preview.url"
                frameborder="0"
            ...
            <!-- 本地视频预览（video标签） -->
            <div
              class="video-local-preview-container"
              v-else-if="preview.isLocal && preview.url"
            >
  ...
  <el-dialog v-model="linkDialogVisible" title="网站分享" width="400px">
    <div class="link-form">
      ...
      <div style="display: flex; align-items: center; gap: 8px">
        <el-input
          v-model="formData.content.link!.favicon"
          placeholder="网站图标"
          style="flex: 1"
        />
  ...
  <el-dialog v-model="musicDialogVisible" title="动态音乐" width="400px">
    <div class="music-form">
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 12px">
        <el-input
          v-model="formData.content.music!.id"
          placeholder="音乐ID"
          style="flex: 1"
        />
        <el-button
          type="primary"
          :loading="fetchingMusic"
...
const otherContentPreviews = computed(() => {
  ...
  if (formData.content.music?.id) {
    ...
    const serverLabel = info?.server
      ? MUSIC_LABELS.server[info.server]
      : music.server
    ...
    previews.push({
      ...
      cover: info?.pic || '',
      musicType: info
        ? `${serverLabel} · ${typeLabel}`
        : `${music.server} - ${music.type}`
    })
  }
  if (videoItem.value) {
...
const VIDEO_PLATFORMS = {
  bilibili: {
    ...
    getIframeSrc: (id: string) =>
      `//player.bilibili.com/player.html?isOutside=true&bvid=${id}&p=1`
  },
  youtube: {
    name: 'YouTube',
...
const getVideoIframeSrc = (platform: string, videoId: string) =>
  VIDEO_PLATFORMS[platform as keyof typeof VIDEO_PLATFORMS]?.getIframeSrc(
    videoId
  ) || ''
...
const cleanupVideoBlob = () => {
  if (
    videoItem.value?.type === 'file' &&
    videoItem.value.url.startsWith('blob:')
  ) {
...
const addVideoUrl = async () => {
  ...
  try {
    ...
    videoUrlInput.value = ''
    ElMessage.success(
      `已识别：${getPlatformName(result.platform)} - ${result.video_id}`
    )
  } catch (error) {
...
const handleImageError = (e: Event) => {
  ...
  parent.innerHTML =
    '<span style="color: #c0c4cc; font-size: 12px;">加载失败</span>'
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (formData.content.text?.trim())
      content.text = formData.content.text.trim()
    if (formData.content.tags?.trim())
      content.tags = formData.content.tags.trim()
    if (uploadedImages.length) content.images = uploadedImages
    if (formData.content.location?.trim())
      content.location = formData.content.location.trim()
